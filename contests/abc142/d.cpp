...
int main() {
    ...
    rep(i,divisions.size()){
        if( is_prime(divisions[i], memo) ){
            ans++;
            prime_divisions.push_back(divisions[i]);
            sets.insert(divisions[i]);
        }
    }


    rep(i,prime_divisions.size()){
        Int target = gcd/prime_divisions[i];
        if ( is_prime(target,memo) ) {
            sets.insert(target);
        }
